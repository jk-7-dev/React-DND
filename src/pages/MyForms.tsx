import { useNavigate, Link } from '@tanstack/react-router'; // <--- CHANGED
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {savedForms.map((form) => (
              <div key={form.id} className="bg-white p-6 rounded-xl shadow-sm border hover:shadow-md transition-all">
                ...
                <p className="text-sm text-gray-500 mb-6">{form.elements.length} questions</p>
                
                {/* Type-safe Navigation */}
                <button 
                  onClick={() => navigate({ to: '/form/$formId', params: { formId: form.id } })} // <--- CHANGED
